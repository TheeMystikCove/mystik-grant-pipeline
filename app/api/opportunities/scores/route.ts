...
// GET /api/opportunities/scores?ids=id1,id2 — return scores map for given opportunity IDs
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    console.error("[opportunities/scores GET]", message);
    return NextResponse.json({}, { status: 500 });
  }
}

// POST /api/opportunities/scores — batch AI-score unscored opportunities
const BATCH_SIZE = 3;
...
export async function POST(_req: NextRequest) {
  ...
  try {
    ...
    console.error("[opportunities/scores POST]", msg);
...
async function handleRescore() {
  ...
  const { data: scored, error: scoreReadErr } = await db
    .from("opportunity_scores")
  ...
  for (let i = 0; i < unscored.length; i += BATCH_SIZE) {
    ...
    const results = await Promise.allSettled(
      batch.map((opp: typeof unscored[number]) => scoreOne(db, opp))
    );
    for (const r of results) {
      if (r.status === "fulfilled") {
        ...
        errors.push(msg);
        console.error("[opportunities/scores POST] failure:", msg);
      }
    }
  }
...
async function scoreOne(db: ReturnType<typeof createAdminClient>, opp: OppRow) {
  ...
  const { error: insErr } = await db.from("opportunity_scores").insert({
    opportunity_id: opp.id,
